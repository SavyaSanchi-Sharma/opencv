...
// FP16/BF16 convolution, C0 == 8. 16-bit storage, FP32 accumulation: the operands are
// expanded to v_float32 on load and packed back on store, so the MAC chain runs on the
// same FMA lanes the CV_32F kernels use. Lives here rather than in conv2_kernels.cpp so
// the CPU dispatcher builds it with -mavx2 -mfma -mf16c; at the -msse3 baseline the
// hfloat <-> float conversions fall back to software bit-twiddling.
                       ...
                       const float* scale__, const float* bias__)
{
    ...
    // The K0 output channels of a block are contiguous only when the group base is
    // K0-aligned; otherwise fall back to the scalar tail below.
    const bool vec_ok = (nlanes <= K0) && (K0 % nlanes == 0);
#else
    const bool vec_ok = false;
#endif

    parallel_for_(Range(0, N*ngroups*Kblk), [&](const Range& r) {
        std::vector<float> coefbuf(K0*3);
        float* scvec = coefbuf.data();
        float* bvec  = scvec + K0;
        float* avec  = bvec + K0;
        ...
        for (int task = r.start; task < r.end; task++) {
            ...
            for (int k0 = 0; k0 < K0; k0++) {
                ...
                const bool valid = k0 < k0end;
                scvec[k0] = valid ? (scale__ ? scale__[k] : 1.f) : 0.f;
                bvec[k0]  = valid ? (bias__ ? bias__[k] : 0.f) : 0.f;
                avec[k0]  = valid ? (fastActivation == FAST_ACTIV_PRELU ? activParams[k] : defaultAlpha) : 0.f;
            }

            const bool contiguous_k = (kbase % K0) == 0;
                    ...
                    for (int x0 = 0; x0 < W; x0++) {
...
#if (CV_SIMD || CV_SIMD_SCALABLE)
                        if (vec_ok && contiguous_k) {
                            const size_t outbase =
                                ((size_t)n*K1 + kbase/K0)*outplane + spatofs;
                            for (int ko = 0; ko < K0; ko += nlanes) {
                                ...
                                for (int i = 0; i < ksize; i++) {
                                    ...
                                    for (int c = 0; c < Cg; c++) {
                                        ...
                                        const _Tp* w = wi + (size_t)c1w*C0*K0 + (size_t)c0w*K0 + ko;
                                        acc = v_fma(vx_setall_f32(av), vx_load_expand(w), acc);
                                    }
                                }

                                acc = v_fma(acc, vx_load(scvec + ko), vx_load(bvec + ko));
                                if (res) {
                                    v_float32 rv = vx_load_expand(res + outbase + ko);
                                    acc = v_add(acc, rv);
                                }
                                v_float32 z = vx_setzero_f32();
                                v_float32 alpha = vx_load(avec + ko);
                                acc = v_min(v_select(v_ge(acc, z), acc, v_mul(acc, alpha)),
                                            vx_setall_f32(maxval));
                                v_pack_store(out + outbase + ko, acc);
                            }
                            continue;
                        }
#endif
                        {
                            ...
                            for (int k0 = 0; k0 < k0end; k0++) {
                                ...
                                float acc = accbuf[k0]*scvec[k0] + bvec[k0];
                                if (res)
                                    acc += (float)res[outofs];
                                acc = acc >= 0.f ? acc : acc*avec[k0];
