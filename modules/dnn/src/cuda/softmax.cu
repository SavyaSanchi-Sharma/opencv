...
#pragma unroll
    ...
    template <class T>
    void softmax(const Stream& stream, Span<T> output, View<T> input, int axis_size, int outer_size, bool log_softmax) {
    ...
    template void softmax(const Stream&, Span<__half>, View<__half>, int, int, bool);
#endif
    template void softmax(const Stream&, Span<float>, View<float>, int, int, bool);
