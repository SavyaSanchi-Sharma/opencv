    ...
    namespace raw {
            ...
            View<T> input, size_type output_size, Span<T> output, std::int64_t* p_indices)
        {
            for (auto id : grid_stride_range(output_size)) {
                ...
                // seed from the clamped window start so an empty window still leaves valid indices
                std::int64_t d_index_max = csl::device::min<std::int64_t>(d_start, depth  - 1);
                std::int64_t w_index_max = csl::device::min<std::int64_t>(w_start, width  - 1);
                std::int64_t h_index_max = csl::device::min<std::int64_t>(h_start, height - 1);
                T maxval = csl::device::numeric_limits<T>::lowest();
